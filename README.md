...
기본적인 데이터베이스에 대한 이해는 전보다 확실히 향상되었다. (도와주신 지훈님께 감사)

workbench의 도움으로 비교적 DB구축을 쉽게 했다.

사용한 sql문은 그다지 어려운 수준은 아니었다. ( 간단한 INSERT, UPDATE, DELETE, SELECT 수준.. )

ER 다이어그램을 이해할 수 있게 된게 가장 기쁘다. ( 경우에 따라 테이블을 새로 만들고 .. 외래키 참조하고 .. )

올해 초부터 장고를 공부하며 db를 접했지만, 이제서야 외래키를 조금 이해하게 되었다. ( ㄷ ㄷ ㄷ )
